...
console.log(responseContent);
   ...
   const sendChannelMessage = async(message) =>{
    ...
    if(channel && channel.members){
        ...
        const pseudonym = isAnonymous
            ? (channel.pseudonymMap.get(senderId) || "unknown")
            : null;

        // emit to each member with pseudonym masking
        channel.members.forEach((member) => {
            ...
            if (memberSocketId) {
                if (isAnonymous) {
                    ...
                    io.to(memberSocketId).emit("receive-channel-message", maskedData);
                } else {
        ...
        // admin always gets real identity + pseudonym
        const adminSocketId = userSocketMap.get(channel.admin.toString());
        if (adminSocketId) {
            const adminData = isAnonymous ? { ...finalData, pseudonym } : finalData;
            io.to(adminSocketId).emit("receive-channel-message", adminData);
        }
    }
   }
