...
export default function MessageContainer() {
  ...
  const renderChannelMessages = (message) => {
    ...
    const isAdmin = selectedChatData.admin?.toString() === userInfo.id?.toString() ||
                    selectedChatData.admin?._id?.toString() === userInfo.id?.toString();
    const isOwnMessage = message.sender._id?.toString() === userInfo.id?.toString();
    ...
    if (!isAnonymousChannel) {
      ...
      // admin sees: pseudonym (Real Name)
      displayName = message.pseudonym
        ? `${message.pseudonym} (${message.sender.firstName} ${message.sender.lastName})`
        : message.sender.firstName;
    } else {
      // regular member sees only pseudonym — backend already replaced firstName with it
      displayName = message.sender.firstName || message.pseudonym || "unknown";
    }

    return (
      <div
        className={`mt-5 ${!isOwnMessage ? "text-left" : "text-right"}`}
      >
        {message.messageType === "text" && (
          <div
            className={`${
